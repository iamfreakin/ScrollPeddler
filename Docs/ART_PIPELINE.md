...
```text
SourceAssets/Props/<Category>/<Asset>/
├─ <Asset>.blend                         작업 원본
└─ Exports/
...
Content/Art/Props/<Category>/Blockouts/
└─ SM_<Asset>.uasset
```
...
- collision은 `UCX_`에서 온 convex 1개만 허용하고 자동 생성 primitive가 섞이면 실패시킨다. `SOCKET_` helper가 Unreal Static Mesh socket으로 반입됐는지도 확인한다.
...
단일 기준은 `Scripts/AssetPipeline/assets/scroll_pickup.json`이다.

- 원본: `SourceAssets/Props/Scrolls/ScrollPickup/ScrollPickup_Blockout.blend`
- 게시물: `SourceAssets/Props/Scrolls/ScrollPickup/Exports/SM_ScrollPickup_Blockout.fbx`
- Unreal: `/Game/Art/Props/Scrolls/Blockouts/SM_ScrollPickup_Blockout`
- 중앙 피벗, +Z up, identity transform
- 예상 root-local 크기: 약 `48.8 × 16.5 × 17.9cm`
- 슬롯: `Paper`, `Binding`, `Seal`, `Rune`
- collision: `UCX_SM_ScrollPickup_Blockout_00`
- socket: `SOCKET_SM_ScrollPickup_Blockout_00`
...
& .\Scripts\AssetPipeline\build_asset.ps1 `
  -Manifest .\Scripts\AssetPipeline\assets\scroll_pickup.json
```
