...
| 인벤토리 | 손 1칸+가방 4칸, 재료 10개 스택, 대형 화물, 줍기·교환·버리기, 정의 기반 월드 외형, 리비전·재전송·동시 claim 방어 | 드래그·분할·정렬·비교 UI, 대형 화물 UX |
...
다중 PIE에서는 각 플레이어 창에서 `SPReady true`를 한 번씩 실행하고 HUD의 런 단계가 `Expedition`인지 확인한 뒤 필드 상호작용을 테스트합니다. `Preparing`에서는 획득·스크롤 사용·탈출 요청이 서버에서 거부되는 것이 정상입니다.
...
이 전체 검증 기록은 후속 `Preparing` 권위 가드와 드롭 메시 수정 이전 기준이다. 후속 변경은 UnrealHeaderTool과 C++/Unity 컴파일을 통과했지만, 열려 있는 에디터가 모듈 DLL을 점유하고 있어 최종 링크·자동화·패키지 smoke 재실행이 남아 있다.
